...
void init_syscall(void) // 1번 인자 : 함수의 인자 수,2번 인자 : 함수명,3번 인자 : 인자의 수
{
	...
	REGSYS(SYS_FCNTL, do_fcntl,3); // fcntl을 syscall에 추가
}
...
int fcntl(int fd,int cmd,long arg) // because of 3 arguments, it has to call syscall3
{
	return syscall3(SYS_FCNTL,fd,cmd,arg); // 인자가 3개이므로
}
