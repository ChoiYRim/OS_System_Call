...
int generic_read(int fd, void *buf, size_t len)
{
	...
	if( (cursor = cur_process->file[fd]) == NULL)
		return -1; // 파일이 열려있지 않다면 에러처리

	if ((~cursor->flags & O_RDONLY) && (~cursor->flags & O_RDWR)){
		return -1; // mode가 읽기 전용 혹은 읽기-쓰기 모드가 아니라면 에러처리
	}
	/* 읽을 길이+현재 오프셋이 현재 프로세스가 사용중인 파일의 크기보다 크다면 읽을 길이 값(len)을 파일의 크기 - 현재 오프셋위치의 값으로 설정 */	
	if (*pos + len > cursor->inode->sn_size) 
		len = cursor->inode->sn_size - *pos;

	/* 현재 프로세스가 사용중인 파일의 inode값, 그 파일의 오프셋, 읽을 버퍼, 버퍼 크기 */
...
int generic_write(int fd, void *buf, size_t len)
{
	...
	if( (cursor = cur_process->file[fd]) == NULL) // 파일이 열려있지 않다면 에러처리
		return -1;

	if ((~cursor->flags & O_WRONLY) && (~cursor->flags & O_RDWR)) // write에 해당하는 플래그 설정이 아니라면 에러처리
	{
		return -1;
	}
